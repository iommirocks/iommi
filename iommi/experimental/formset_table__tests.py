...
from iommi.experimental.formset_table import (
    FormsetColumn,
    FormsetTable,
)
...
def test_formset_table():
    formset_table = FormsetTable(
        columns=dict(
            editable_thing=FormsetColumn(
                edit=Namespace(call_target=Field),
            ),
            readonly_thing=FormsetColumn(),
        ),
        rows=[
    ...
    verify_table_html(
        table=formset_table.bind(request=req('get')),
        # language=html
        expected_html="""
