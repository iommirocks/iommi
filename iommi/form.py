    ...
    def choice_queryset(cls, choices: QuerySet = None, **kwargs):
        if 'model' not in kwargs:
            if isinstance(choices, QuerySet):
                ...
                assert False, (
                    'The convenience feature to automatically get the parameter model set only works for '
                    'QuerySet instances or if you specify model_field'
                )
    ...
    def on_refine_done(self):
        ...
        if self.auto:
            ...
            assert not self.model, (
                "You can't use the auto feature and explicitly pass model. Either pass auto__model, "
                "or we will set the model for you from auto__instance"
            )
