class IommiBase {
    ...
    onDOMLoad() {
        ...
        if (urlParams.has('_iommi_code_finder')) {
            ...
            window.iommiCodeFinderActive = true;
        }
    }
...
function createIommiCodeFinderOverlay() {
    ...
    // Set isActive based on global state (set when _iommi_code_finder parameter is present)
    window.iommiCodeFinderActive = window.iommiCodeFinderActive || false;

    // Handle clicks when overlay is active
    document.addEventListener('click', (e) => {
        if (!window.iommiCodeFinderActive)
            return;
    ...
    window.addEventListener('scroll', () => {
        if (window.iommiCodeFinderActive && highlight.style.display === 'block' && lastTarget) {
